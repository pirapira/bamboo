...
let hex_of_instruction (i : Wrap_bn.t instruction) : Hexa.hex =
...
let rev_append_op (h : Hexa.hex) (i : Wrap_bn.t instruction) : Hexa.hex =
  Hexa.concat_hex (hex_of_instruction i) h

let hex_of_program (p : Wrap_bn.t program) : Hexa.hex =
  List.fold_left rev_append_op Hexa.empty_hex p

let print_imm_program (p : Wrap_bn.t program) : unit =
  let hex = hex_of_program p in
  Hexa.print_hex ~prefix:"0x" hex

let string_of_imm_program (p : Wrap_bn.t program) : string =
  let hex = hex_of_program p in
